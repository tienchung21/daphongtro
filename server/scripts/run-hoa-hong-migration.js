 ...
 * Chạy từ thư mục server: node scripts/run-hoa-hong-migration.js
 */
...
async function runMigration() {
  ...
  try {
    ...
    // Đọc file migration
    const migrationFile = path.join(__dirname, '..', '..', 'migrations', '2025_11_06_add_hoa_hong_to_duan.sql');
    ...
    const [dataCheck] = await connection.query(`
      SELECT 
        COUNT(*) as total,
        COUNT(TrangThaiDuyetHoaHong) as has_value,
        COUNT(*) - COUNT(TrangThaiDuyetHoaHong) as null_count
      FROM duan
    `);
