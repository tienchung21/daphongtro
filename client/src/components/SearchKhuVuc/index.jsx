...
function SearchKhuVuc({ onSearch }) {
  ...
  const loadTree = async () => {
    ...
    try {
      ...
      const initLevels = Array.from({ length: depth }, (_, i) => (i === 0 ? raw : []));
  ...
  const handleSelect = (levelIndex, id) => {
    ...
    for (let i = levelIndex + 1; i < newSelected.length; i++) newSelected[i] = '';
    ...
    for (let lvl = 0; lvl < newLevels.length; lvl++) {
      ...
      if (lvl > 0) {
        ...
        const parent = nodeList.find(n => (n.KhuVucID ?? n.id) === Number(prevId));
  ...
  return (
    <div className="search-khuvuc">
      <div className="sk-row">
        ...
        {!loading && levels.map((opts, idx) => (
          <select
            key={idx}
            className="sk-select"
            value={selectedIds[idx] ?? ''}
            onChange={(e) => handleSelect(idx, e.target.value)}
            disabled={idx !== 0 && !(selectedIds[idx - 1])} /* disabled until parent chosen */
          >
            <option value="">{idx === 0 ? 'Chọn khu vực ' : 'Chọn...'}</option>
            {opts && opts.length ? opts.map(opt => {
        ...
        <button className="sk-btn" onClick={handleSearch}>Tìm</button>
      </div>
    </div>
  );
}
